...
class RosImageCollector(Node):
    def __init__(self, camera_names, topic_prefix: str, store: _FrameStore):
        super().__init__("rfm_ros_mjpeg_collector")
        self.store = store
...
<head>
  ...
  <title>RFM Camera Streams</title>
  <style>
...
def main():
    ...
    ap.add_argument("--camera-map-file", default="/home/lcw/RFM_lerobot/camera_port_map.json")
    ...
    try:
        print(f"[rfm-web] Serving MJPEG on http://{args.bind}:{args.port}/  (bind={args.bind})")
