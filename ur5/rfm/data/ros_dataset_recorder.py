...
def _sanitize_task_name(task_name: str) -> str:
    s = str(task_name).strip().replace(" ", "_")
...
class RosDatasetRecorder(Node):
    def __init__(self, args: Args):
        ...
        base = Path(os.path.expanduser(args.out_dir))
        task = _sanitize_task_name(args.task_name)
        # Preferred layout:
        #   <out_dir>/<task_name>/episode_000/*.pkl
        # If task_name is empty, keep timestamp folder fallback.
        run_name = task if task else ts
...
def main():
    ...
    p.add_argument("--task-name", default=Args.task_name)
    ...
    args = Args(
        ...
        task_name=str(args_ns.task_name),
        hz=float(args_ns.hz),
